  ...
  function applyAthleteHistory(payload,type){
    ...
    for(const item of list){
      ...
      events.forEach((eventName,i)=>{
        const rows=(Array.isArray(item.events[eventName])?item.events[eventName]:[]).filter(r=>Number.isFinite(Number(r.mark))).filter(r=>['2025','2026'].includes(String(r.year||''))).filter(r=>!isCurrentBirmingham(r)).sort((a,b)=>Date.parse(String(b.date||''))-Date.parse(String(a.date||''))||String(b.venue||'').length-String(a.venue||'').length);
