(function(){
  ...
  function restoreSelection(saved){
    ...
    if(selectedName()===saved.name)return true;
  ...
  function restore(){
    ...
    setTimeout(()=>{restoring=false;},900);
  }
  ...
  document.addEventListener('click',ev=>{
    ...
    if(pick)write({name:pick.dataset.name,values:{},updatedAt:Date.now()});
    if(ev.target.closest?.('#whatIfApply'))setTimeout(capture,0);
  ...
  function start(){mo.observe(document.body,{subtree:true,childList:true});schedule(300);}
