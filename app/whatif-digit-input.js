(function(){
  ...
  function isCompleteOrEmpty(input){
    const n=digitCount(input);
    return n===0||n===requiredDigits(eventNameFor(input));
  }
  function validate(input){
    const event=eventNameFor(input),req=requiredDigits(event),digits=digitCount(input);
    const ok=digits===0||digits===req;
    input.dataset.exactDigits=ok?'1':'0';
    input.style.borderColor=ok?'#456783':'#ff9f43';
    input.title=ok?'':`Skriv nøyaktig ${req} tall før du går videre`;
    return ok;
  }
  function updateApply(){
    ...
    apply.disabled=bad;apply.style.opacity=bad?'0.45':'1';apply.style.cursor=bad?'not-allowed':'pointer';
  ...
  function move(input,dir){
    const list=fields(),i=list.indexOf(input),next=list[i+dir];
    if(next){next.focus();try{next.setSelectionRange(next.value.length,next.value.length);}catch(_e){}}
  }
  function blockLeave(input){
    if(isCompleteOrEmpty(input))return false;
    validate(input);input.focus();try{input.setSelectionRange(input.value.length,input.value.length);}catch(_e){}
    return true;
  }
  function upgrade(input){
    ...
    input.placeholder=placeholderFor(event);
    ...
    input.addEventListener('keydown',ev=>{
      if(ev.key==='ArrowDown'||ev.key==='ArrowUp'||ev.key==='Tab'){
        if(blockLeave(input)){ev.preventDefault();ev.stopPropagation();return;}
        if(ev.key==='ArrowDown'||ev.key==='ArrowUp'){
          ev.preventDefault();move(input,ev.key==='ArrowDown'?1:-1);
        }
      }
    });
    input.addEventListener('blur',()=>{
      if(!isCompleteOrEmpty(input))setTimeout(()=>{if(document.querySelector('#modal.open'))blockLeave(input);},0);
    });
    normalize(input);
  }
