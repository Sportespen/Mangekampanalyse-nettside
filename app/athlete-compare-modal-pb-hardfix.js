(function(){
  ...
  function summaryBox(content){
    ...
    const existing=content.querySelector('.basis-summary-box');
    if(existing)return existing;
    const label=[...content.querySelectorAll('small')].find(x=>x.textContent.trim()==='Forventet resultat');
    const box=label?.parentElement||null;
    if(box)box.classList.add('basis-summary-box');
    return box;
  }
  async function apply(){
    ...
    const expected=(box.dataset.expectedValue||box.querySelector('.basis-expected-value')?.textContent||box.querySelector(':scope > b')?.textContent||box.querySelector('b')?.textContent||'—').trim();
    box.dataset.expectedValue=expected;
    const card=modal.querySelector('.modal-card');
    if(card){
      card.style.setProperty('width','min(650px,calc(100vw - 24px))','important');
      card.style.setProperty('max-width','650px','important');
      card.style.setProperty('height','fit-content','important');
      card.style.setProperty('min-height','0','important');
      card.style.setProperty('max-height','calc(100vh - 24px)','important');
      card.style.setProperty('padding','14px 16px 16px','important');
    ...
    box.style.setProperty('background','#102a45','important');
    box.style.setProperty('border-radius','8px','important');
    box.style.setProperty('display','grid','important');
    box.style.setProperty('grid-template-columns','1fr 1fr','important');
    box.style.setProperty('align-items','end','important');
    box.style.setProperty('gap','16px','important');
    box.style.setProperty('height','auto','important');
    box.style.setProperty('min-height','0','important');
    box.style.setProperty('max-height','none','important');
    box.style.setProperty('overflow','visible','important');
    if(!box.querySelector(':scope > .basis-expected-col')){
    ...
    const target=summaryBox(content)?.querySelector(':scope > .basis-pb-col .basis-pb-value');if(target)target.textContent=fmt(id.ev,pb);
  }
  ...
  document.addEventListener('click',()=>{cleanRow();schedule();setTimeout(apply,150);setTimeout(apply,500);},true);
  new MutationObserver(muts=>{cleanRow();if(!document.querySelector('#modal')?.classList.contains('open'))return;if(muts.some(m=>[...m.addedNodes].some(n=>n.nodeType===1&&(n.id==='modalContent'||n.querySelector?.('#modalContent,h2,.basis-summary-box')))))schedule();}).observe(document.documentElement,{childList:true,subtree:true});
  cleanRow();
})();
