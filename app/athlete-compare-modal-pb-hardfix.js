(function(){
  ...
  function summary(content){const original=[...content.children].find(el=>el.tagName==='DIV'&&el.querySelector(':scope > small')?.textContent.trim()==='Forventet resultat');return original||content.querySelector('.basis-summary-box')||null;}
  async function apply(){
    ...
    const box=summary(content);if(!box)return;
    const expected=(box.querySelector('.basis-expected-value')?.textContent||box.querySelector(':scope > b')?.textContent||box.querySelector('b')?.textContent||'—').trim();
    ...
    box.className='basis-summary-box';
  ...
  document.addEventListener('click',ev=>{
    if(ev.target.closest?.('[data-basis]')){const c=document.querySelector('#modalContent');if(c)delete c.dataset.pbPopupDone;setTimeout(apply,0);setTimeout(apply,80);setTimeout(apply,250);}
  },true);
})();
