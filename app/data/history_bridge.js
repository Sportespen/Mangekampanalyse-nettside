(function(){
  ...
  const addOne=(name,event,mark,venue,year)=>{H[name]=H[name]||{};H[name][event]=H[name][event]||[];const display=(event==='800m'||event==='1500m')?`${Math.floor(mark/60)}:${(mark%60).toFixed(2).padStart(5,'0')}`:Number(mark).toFixed(2);if(!H[name][event].some(r=>Number(r[0])===Number(mark)))H[name][event].unshift([mark,display,venue,String(year)]);};
  ...
  for(const hist of Object.values(H)){if(!hist)continue;for(const [event,rows] of Object.entries(hist)){if(!Array.isArray(rows))continue;const seen=new Set();hist[event]=rows.filter(r=>{if(!Array.isArray(r)||!Number.isFinite(Number(r[0])))return false;const key=Number(r[0]).toFixed(3);if(seen.has(key))return false;seen.add(key);return true;}).slice(0,4);}}
