(function(){
  ...
  function resultView(){
    ...
    content.querySelector('#whatIfAdjust')?.addEventListener('click',()=>{content.querySelectorAll('[data-whatif]').forEach(inp=>inp.disabled=false);summary.remove();const holder=content.querySelector('#whatIfAdjust')?.parentElement;if(holder)holder.innerHTML=`<button id="whatIfReset" type="button" style="padding:10px 14px;border:1px solid #456783;border-radius:8px;background:#0d2743;color:#fff;font-weight:800;cursor:pointer">Nullstill</button><button id="whatIfApply" type="button" style="padding:10px 14px;border:0;border-radius:8px;background:#6f4cff;color:#fff;font-weight:900;cursor:pointer">Bruk scenario</button>`;});
    content.querySelector('#whatIfReset2')?.addEventListener('click',()=>{sessionStorage.removeItem(`mangekamp_compare_state_${typeof currentType==='string'?currentType:'men'}`);content.querySelectorAll('[data-whatif]').forEach(inp=>{inp.disabled=false;inp.value='';inp.dispatchEvent(new Event('input',{bubbles:true}));});document.querySelector('#whatIfBtn')?.click();});
