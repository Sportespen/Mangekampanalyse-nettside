(function(){
  ...
  function predictedMark(athlete,eventIndex){return predict(athlete,eventIndex);}
  function hasActual(athlete,eventIndex){return Number.isFinite(Number(athlete.liveActual?.[eventIndex]));}
  function actualMark(athlete,eventIndex){const v=Number(athlete.liveActual?.[eventIndex]);return Number.isFinite(v)?v:null;}
  function eventIsCompleted(eventName){const s=String(liveForType().eventStatus?.[eventName]||'').toLowerCase();return s==='finished'||s==='official';}
  ...
  function renderLiveForecast(){
    ...
    const rows=(D.athletes||[]).map(athlete=>{
      ...
      const actualPts=D.events.reduce((sum,eventName,i)=>{const v=actualMark(athlete,i);return sum+(eventIsCompleted(eventName)&&v!=null?scoreEvent(i,v):0);},0);
    ...
    body.innerHTML=currentRows.map(r=>{
      const cr=currentRank.get(r.athlete.name),fr=forecastRank.get(r.athlete.name),change=cr-fr;
      const cells=r.vals.map((v,i)=>{const actual=hasActual(r.athlete,i);const cls=actual?'actual':'pred';const title=actual?'Faktisk resultat fra arrangørens live-resultater':'Prognose: snitt av de tre beste av de fire siste resultatene';return `<td class="${cls}" title="${title}">${v==null?'—':displayMark(D.events[i],v)}</td>`;}).join('');
  ...
  function syncLive(){applyLiveToAthletes();updateStatus();renderLiveForecast();}
