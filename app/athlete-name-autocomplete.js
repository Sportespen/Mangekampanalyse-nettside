(function(){
  if(window.__athleteNameAutocompleteV4)return;
  window.__athleteNameAutocompleteV4=true;
  ...
  function loadIndex(){if(!indexPromise)indexPromise=fetch('/app/data/athlete-name-index.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('index');return r.json();});return indexPromise;}
  function startSearch(input){input.focus();input.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',bubbles:true,cancelable:true}));}
  async function choose(a,input,results){
    input.value=a.name;results.style.display='none';startSearch(input);
  ...
  function render(list,input,results,status){
    ...
    [...results.querySelectorAll('[data-suggest-id]')].forEach((b,i)=>b.onclick=()=>choose(list[i],input,results));
  }
  async function suggest(input,results,status){
    ...
    try{const idx=await loadIndex();if(my!==seq)return;const source=Array.isArray(idx?.[type()])?idx[type()]:[];const list=source.filter(a=>a?.name&&matches(a.name,q)).sort((a,b)=>rank(a.name,q)-rank(b.name,q)||a.name.localeCompare(b.name,'nb')).slice(0,60);if(list.length){render(list,input,results,status);return;}}catch(_){}
    try{const r=await fetch(`/api/athlete-name-search?type=${encodeURIComponent(type())}&q=${encodeURIComponent(q)}`,{cache:'no-store'}),d=await r.json();if(my!==seq)return;render(Array.isArray(d.athletes)?d.athletes:[],input,results,status);}catch(_){if(my===seq){results.style.display='none';if(status)status.textContent='Søket kunne ikke fullføres.';}}
  }
  function bind(){
    const input=document.querySelector('#athleteCompareSearch'),results=document.querySelector('#athleteCompareResults'),status=document.querySelector('#athleteCompareStatus');
    if(!input||!results||input.dataset.autocompleteV4==='1')return;
    input.dataset.autocompleteV4='1';results.style.maxHeight='390px';results.style.overflowY='auto';loadIndex().catch(()=>{});
    input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>suggest(input,results,status),150);});
