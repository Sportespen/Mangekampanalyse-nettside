(function(){
  ...
  document.addEventListener('click',ev=>{
    if(ev.target.closest?.('#athleteCompareResults button'))setTimeout(()=>{remember();mount();},80);
    if(ev.target.closest?.('.tab'))setTimeout(()=>{remember();mount();if(typeof window.renderAthleteComparison==='function')window.renderAthleteComparison();},80);
    if(ev.target.closest?.('#removeCompareBtn'))preserved=null;
  },true);
  function start(){remember();mount();}
