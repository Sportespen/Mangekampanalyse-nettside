...
// No date cutoff here: this must reflect every valid 2025-2026 senior result up to today for
// whichever athlete is looked up, not just those before some fixed date. A prior version reused
// the EM Birmingham 2026 start date (12 Aug) as a global cutoff, which silently excluded any
// result from after that date regardless of event or competition - confirmed live: it dropped
// Jonathan Hertwig-Odegaard's three most recent 400m races (his current PB among them), for no
// reason connected to Birmingham at all.
...
// High jump, shot put and pole vault aren't wind-affected and indoor conditions don't change
// their validity the way an indoor sprint track or indoor long-jump runway can, so indoor results
// count fully equally here - not just as a fallback once outdoor results run short like every
// other event, but merged with outdoor and ranked purely by recency for the 4 kept results.
const INDOOR_EQUAL_EVENTS=new Set(['Høyde','Kule','Stav']);
function finalize(grouped){const out={};for(const[e,rows]of Object.entries(grouped)){const all=uniqueRecent(rows);if(INDOOR_EQUAL_EVENTS.has(e)){out[e]=all.sort((a,b)=>dateValue(b.date)-dateValue(a.date)).slice(0,4);continue;}const outdoor=all.filter(r=>!r.indoor).sort((a,b)=>dateValue(b.date)-dateValue(a.date)),indoor=all.filter(r=>r.indoor).sort((a,b)=>dateValue(b.date)-dateValue(a.date)),kept=outdoor.slice(0,4);if(kept.length<4){for(const r of indoor){if(kept.length===4)break;kept.push(r);}}out[e]=kept;}return out;}
...
async function fetchYearResults(id,year){const res=await fetch(WA_GRAPHQL_ENDPOINT,{method:'POST',headers:{'content-type':'application/json','x-api-key':WA_GRAPHQL_KEY,'x-graphql-client-name':'worldathletics'},body:JSON.stringify({query:RESULTS_QUERY,variables:{id:Number(id),resultsByYear:year,resultsByYearOrderBy:'discipline'}})});if(!res.ok)throw new Error(`graphql ${res.status}`);const payload=await res.json();return payload?.data?.getSingleCompetitorResultsDiscipline?.resultsByEvent||[];}
function rowsFromGraphqlEvents(events){const rows=[];for(const ev of (Array.isArray(events)?events:[])){const discipline=ev?.discipline;for(const r of (ev?.results||[]))rows.push({discipline,mark:r?.mark,date:toIsoDate(r?.date),competition:r?.competition,venue:r?.venue,wind:r?.wind,legal:r?.notLegal!==true,category:r?.category});}return rows;}
// fetchCompetitorFromHtml's resultsByYear is always the CURRENT calendar year only (2026), so
// analyseAthlete() was silently limited to this season alone despite YEARS/the site's own promised
// "opptil fire siste gyldige seniorresultater fra 2025-2026" description - confirmed live: Jonathan
// Hertwig-Odegaard's analyse rows never included any 2025 date. Bring in the previous year too via
// the same GraphQL technique used for the Decastar bulk history, so this matches that promise.
async function resultsForAnalyse(id){const {rows,competitor}=await resultsFor(id);let prevRows=[];try{prevRows=rowsFromGraphqlEvents(await fetchYearResults(id,2025));}catch(_e){}return {rows:rows.concat(prevRows),competitor};}
const COMBINED_TIME_EVENTS=new Set(['100m','400m','110mh','1500m','100mh','200m','800m']);
function computePbsAndCombined(rows,type){const pbs={};let combinedPB=0;for(const r of (Array.isArray(rows)?rows:[])){const disc=String(r?.discipline||'').trim();const wantCombined=type==='women'?/^heptathlon$/i.test(disc):/^decathlon$/i.test(disc);const juniorText=[disc,r?.category].filter(Boolean).join(' ');if(wantCombined&&!/\b(u18|u20|junior|youth)\b/i.test(juniorText)){const n=Number(String(r?.mark??'').replace(/[^0-9.]/g,''));if(Number.isFinite(n)&&n>combinedPB)combinedPB=n;}const ev=appEvent(disc);if(!ev||!EVENTS[type].includes(ev)||!seniorOnly(r,ev,type)||!windLegal(r,ev))continue;const mark=parseMark(r?.mark,ev);if(mark==null)continue;const old=pbs[ev],better=!old||(COMBINED_TIME_EVENTS.has(ev)?mark<old.mark:mark>old.mark);if(better)pbs[ev]={mark,display:String(r?.mark||''),year:yearOf(r?.date),date:String(r?.date||''),venue:venueOf(r),competition:String(r?.competition||''),wind:String(r?.wind??''),indoor:indoorOf(r)};}return {pbs,combinedPB:combinedPB||null};}
...
async function analyseAthlete(id,type,nameHint){const {rows,competitor}=await resultsForAnalyse(id);const meta=competitor?metaFromCompetitor(competitor):{name:'',nation:'',birth:'',gender:''};
