...
function finalize(grouped){const out={};for(const[e,rows]of Object.entries(grouped)){const seen=new Set();out[e]=rows.sort((a,b)=>dateValue(b.date)-dateValue(a.date)).filter(r=>{if(!YEARS.includes(Number(r.year)))return false;if(!String(r.venue||'').trim())return false;const k=[r.mark,r.date,r.venue,r.competition].join('|');if(seen.has(k))return false;seen.add(k);return true;}).slice(0,4);}return out;}
async function athleteHistory(name,type){try{
  ...
  for(const r of batches.flat()){
    ...
    const venue=String(r.location||r.venue||'').trim();if(!venue)continue;
    pushRow(grouped,e,{mark,display:String(r.mark||''),venue,year:yearOf(r.date),date:String(r.date||''),competition:String(r.competition||r.meeting||''),wind:String(r.wind??r.windReading??''),legal:true,source:'world-athletics'});
  }
...
export async function onRequestGet({request}){const u=new URL(request.url);const type=u.searchParams.get('type')==='women'?'women':'men';const names=type==='women'?WOMEN:MEN;try{const data=await mapLimited(names,3,n=>athleteHistory(n,type));const coverage=data.map(a=>({name:a.name,events:Object.fromEntries(Object.entries(a.events||{}).map(([e,r])=>[e,r.length])),error:a.error||null}));return new Response(JSON.stringify({source:'World Athletics verified 2025-2026 venue data',updatedAt:new Date().toISOString(),type,athletes:data,coverage}),{headers:{'content-type':'application/json; charset=utf-8','cache-control':'public, max-age=60'}});}catch(e){return new Response(JSON.stringify({error:String(e?.message||e)}),{status:502,headers:{'content-type':'application/json'}});}}
