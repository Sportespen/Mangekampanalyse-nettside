...
async function eventRows(phaseId,groupIds=[]){const requests=[
  query('liveResults.getCombinedEventResultsFeed',{event:phaseId,competitionCode:COMPETITION_CODE,isSummary:true},true).catch(()=>null),
  query('liveResults.getCombinedEventResultsFeed',{event:phaseId,competitionCode:COMPETITION_CODE,isSummary:false},true).catch(()=>null),
  ...groupIds.map(groupId=>query('liveResults.getCombinedEventResultsFeed',{event:groupId,competitionCode:COMPETITION_CODE,isSummary:false},true).catch(()=>null))
];
  const payloads=await Promise.all(requests),byAthlete=new Map(),anonymous=[];
  for(const payload of payloads){for(const row of rowsFromPayload(payload)){const id=String(row?.athleteId||row?.federationId||'');if(!id){anonymous.push(row);continue;}const prev=byAthlete.get(id)||{};const next={...prev,...row};const attempts=cleanAttempts(row?.attempts);if(attempts.length)next.attempts=attempts;else if(Array.isArray(prev.attempts)&&prev.attempts.length)next.attempts=prev.attempts;byAthlete.set(id,next);}}
  return[...byAthlete.values(),...anonymous];
}
async function collectSection(defs,athletes,statuses){const results={},eventStatus={},eventHasMarks={};const all=await Promise.all(defs.map(async([eventName,phaseId,groupIds=[]])=>({eventName,status:statuses[phaseId]||'',rows:await eventRows(phaseId,groupIds)})));for(const{eventName,status,rows}of all){eventStatus[eventName]=status;let added=0;for(const row of rows){const id=String(row.athleteId||row.federationId||'');const meta=athletes[id]||{};const name=meta.name||cleanName(row.fullName||row.name||row.athleteName||'');const raw=row.result??row.performance??row.mark??row.resultValue;const mark=parseMark(eventName,raw);const terminal=rawStatus(raw)||rawStatus(row.status)||rawStatus(row.resultStatus)||rawStatus(row.athleteStatus);if(!name||(mark==null&&!terminal))continue;const entry=results[name]||={};if(meta.nation)entry.nation=meta.nation;if(meta.birth)entry.birth=meta.birth;const cr=row.combinedResult&&typeof row.combinedResult==='object'?row.combinedResult:{};const attempts=cleanAttempts(row.attempts);entry[eventName]={mark:mark,display:terminal||String(raw??''),resultStatus:terminal,points:terminal?0:(cr.points??row.points??null),status,wind:row.raceWind||row.bestResultWind||row.wind||'',athleteId:id||null,attempts,bestRoundNo:row.bestRoundNo??null,pointsBefore:cr.pointsBefore??null,pointsAfter:cr.pointsAfter??null,rankBefore:cr.rankBefore??null,rankAfter:cr.rankAfter??null};added++;}eventHasMarks[eventName]=added;}let completedEvents=0;for(const[eventName]of defs){if((eventHasMarks[eventName]||0)>0)completedEvents++;else break;}return{completedEvents,results,eventStatus,eventHasMarks};}
