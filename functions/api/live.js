...
const HORIZONTAL_FIELD = new Set(['Lengde','Kule','Diskos','Spyd']);
...
function cleanAttempts(value){if(!Array.isArray(value))return[];return value.map((a,i)=>{if(a==null)return null;if(typeof a!=='object')return{attemptId:i+1,result:String(a)};const result=a.results??a.result??a.mark??a.value??a.performance??a.distance??a.bestResult??'';if(result==null||String(result).trim()==='')return null;return{attemptId:Number(a.attemptId??a.round??a.roundNo??i+1),result:String(result).trim(),wind:String(a.wind??a.resultWind??'').trim()};}).filter(Boolean);}
function candidateValue(value){if(value==null)return null;if(typeof value!=='object')return value;return value.result??value.performance??value.mark??value.value??value.distance??value.bestResult??value.resultValue??value.bestResultValue??null;}
function rawResult(eventName,row){
  const direct=[row?.result,row?.performance,row?.mark,row?.resultValue,row?.bestResult,row?.bestMark,row?.bestResultValue,row?.bestPerformance,row?.distance];
  for(const value of direct){const c=candidateValue(value);if(c!=null&&String(c).trim()!=='')return c;}
  if(!HORIZONTAL_FIELD.has(eventName))return null;
  const arrays=[row?.attempts,row?.results,row?.trials,row?.series,row?.attemptResults];
  let best=null,bestRaw=null;
  for(const arr of arrays){
    if(!Array.isArray(arr))continue;
    for(const item of arr){
      const c=candidateValue(item);
      const mark=parseMark(eventName,c);
      if(mark!=null&&(best==null||mark>best)){best=mark;bestRaw=c;}
    }
  }
  return bestRaw;
}
...
async function eventRows(phaseId,groupIds=[]){const requests=[query('liveResults.getCombinedEventResultsFeed',{event:phaseId,competitionCode:COMPETITION_CODE,isSummary:true},true).catch(()=>null),query('liveResults.getCombinedEventResultsFeed',{event:phaseId,competitionCode:COMPETITION_CODE,isSummary:false},true).catch(()=>null),...groupIds.map(groupId=>query('liveResults.getCombinedEventResultsFeed',{event:groupId,competitionCode:COMPETITION_CODE,isSummary:false},true).catch(()=>null))];const payloads=await Promise.all(requests),byAthlete=new Map(),anonymous=[];for(const payload of payloads){for(const row of rowsFromPayload(payload)){const id=String(row?.athleteId||row?.federationId||'');if(!id){anonymous.push(row);continue;}const prev=byAthlete.get(id)||{};const next={...prev,...row};if(Array.isArray(row?.attempts)&&row.attempts.length)next.attempts=row.attempts;if(Array.isArray(row?.results)&&row.results.length)next.results=row.results;if(Array.isArray(row?.trials)&&row.trials.length)next.trials=row.trials;if(Array.isArray(row?.series)&&row.series.length)next.series=row.series;if(Array.isArray(row?.attemptResults)&&row.attemptResults.length)next.attemptResults=row.attemptResults;if(Array.isArray(row?.progression)&&row.progression.length)next.progression=row.progression;byAthlete.set(id,next);}}return[...byAthlete.values(),...anonymous];}
async function collectSection(defs,athletes,statuses){const results={},eventStatus={},eventHasMarks={};const all=await Promise.all(defs.map(async([eventName,phaseId,groupIds=[]])=>({eventName,status:statuses[phaseId]||'',rows:await eventRows(phaseId,groupIds)})));for(const{eventName,status,rows}of all){eventStatus[eventName]=status;let added=0;for(const row of rows){const id=String(row.athleteId||row.federationId||'');const meta=athletes[id]||{};const name=meta.name||cleanName(row.fullName||row.name||row.athleteName||'');const raw=rawResult(eventName,row);const mark=parseMark(eventName,raw);const terminal=rawStatus(raw)||rawStatus(row.status)||rawStatus(row.resultStatus)||rawStatus(row.athleteStatus);if(!name||(mark==null&&!terminal))continue;const entry=results[name]||={};if(meta.nation)entry.nation=meta.nation;if(meta.birth)entry.birth=meta.birth;const cr=row.combinedResult&&typeof row.combinedResult==='object'?row.combinedResult:{};const isVertical=eventName==='Høyde'||eventName==='Stav';const attemptSource=row.attempts||row.results||row.trials||row.series||row.attemptResults;const attempts=isVertical?verticalAttempts(row):cleanAttempts(attemptSource);entry[eventName]={mark,display:terminal||String(raw??''),resultStatus:terminal,points:terminal?0:(cr.points??row.points??null),status,wind:row.raceWind||row.bestResultWind||row.wind||'',athleteId:id||null,attempts,attemptMode:isVertical?'vertical':'series',bestRoundNo:row.bestRoundNo??null,pointsBefore:cr.pointsBefore??null,pointsAfter:cr.pointsAfter??null,rankBefore:cr.rankBefore??null,rankAfter:cr.rankAfter??null};added++;}eventHasMarks[eventName]=added;}let completedEvents=0;for(const[eventName]of defs){if((eventHasMarks[eventName]||0)>0)completedEvents++;else break;}return{completedEvents,results,eventStatus,eventHasMarks};}
