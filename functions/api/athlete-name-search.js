...
export async function onRequestGet({request}){
  ...
  try{
    ...
    const body={query:QUERY,variables:{query:q,gender:type==='women'?'female':'male',disciplineCode:type==='women'?'HEP':'DEC',environment:null,countryCode:null}};
    const r=await fetch(endpoint,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json','x-api-key':apiKey},body:JSON.stringify(body),cf:{cacheTtl:60,cacheEverything:true}});
    if(!r.ok)throw new Error(`GraphQL HTTP ${r.status}`);
    const j=await r.json();
    const rows=Array.isArray(j?.data?.searchCompetitors)?j.data.searchCompetitors:[];
    const athletes=rows.map(a=>({id:String(a.aaAthleteId||''),name:[a.givenName,a.familyName].filter(Boolean).join(' ').trim(),nation:String(a.country||''),birth:String(a.birthDate||''),discipline:type==='women'?'Sjukamp':'Tikamp'})).filter(a=>a.id&&a.name&&matches(a.name,q)).sort((a,b)=>score(a.name,q)-score(b.name,q)||a.name.localeCompare(b.name,'nb')).slice(0,40);
