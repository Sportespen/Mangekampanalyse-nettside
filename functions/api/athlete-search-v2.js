...
function windLegal(r,event){if(!WIND_EVENTS.has(event)||indoorOf(r))return true;if(r?.legal===false)return false;const raw=String(r?.wind??r?.windReading??'').trim().replace(',','.');if(!raw)return r?.legal===true||true;const w=Number(raw.replace(/[^0-9.+-]/g,''));return Number.isFinite(w)&&w<=2.0;}
...
async function enrich(data,type){
  ...
  const r=await fetch(`${SOURCE}/athletes/${encodeURIComponent(data.id)}/results`,{headers:{Accept:'application/json','User-Agent':'Mangekampanalyse/2.0'},cf:{cacheTtl:60,cacheEverything:true}}).catch(()=>null);
  if(!r?.ok)return data;
  const rows=await r.json().catch(()=>[]);if(!Array.isArray(rows))return data;
  ...
  for(const row of rows){
    ...
    if(better)pbs[ev]={mark,display:String(row?.mark||''),year:yearOf(row?.date),date:String(row?.date||''),venue:venueOf(row),competition:String(row?.competition||row?.meeting||''),wind:String(row?.wind??row?.windReading??''),indoor:indoorOf(row)};
  }
