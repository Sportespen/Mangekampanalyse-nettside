...
function norm(s){return String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\([^)]*\)/g,'').replace(/\b(stadium|stadion|field|arena|sportzentrum|athletikstadion)\b/gi,' ').replace(/[^a-z0-9]+/gi,' ').trim().toLowerCase();}
...
function finalize(grouped){const out={};for(const[e,rows]of Object.entries(grouped)){const kept=[];for(const r of rows.sort((a,b)=>dateValue(b.date)-dateValue(a.date)||String(b.venue||'').length-String(a.venue||'').length)){if(kept.some(x=>samePerformance(x,r)))continue;kept.push(r);if(kept.length===4)break;}out[e]=kept;}return out;}
